...
from login_func        import read_mongo, get_cursor
...
from components.charts import scatter, pie, table, treemap, sunburst

load_info = toml.load("config.toml")
fpath = "json"

db = config("MONGODB_DATABASE", default="admin")
collection = config("MONGODB_LAYOUT_COLLECTIONS", default="charts")
...
def mongodb_connection(db_name, collection_name):
    ...
    mongodb_database = config("MONGODB_ID_CLIENT", default="mongodb://localhost:27017/")
...
database_server = html.Div(dbc.Select(
    id="select-server-type",
    placeholder="Server",
    options=[
        {"label":i.capitalize(), "value":i} for i in ["mongodb"]
    ],
    # value="bar"
    #className="bottom8"
    ))
...
server_detail = dbc.Stack([
    dbc.Col([
        html.Div("Select the database: "), 
        dbc.Select(
            id="select-database-type",
            options=[
                {"label":i.capitalize(), "value":i} for i in [""]
            ])
        ], lg=8, align='center'),
    dbc.Col([
        html.Div("Select the collection: "), 
        dbc.Select(
...
select_chart_type = html.Div(dbc.Select(
    ...
    options=[
        {"label":i.capitalize(), "value":i} for i in ["bar", "line", "scatter", "pie", "table", "treemap", "sunburst"]
    ],
    # value="bar"
    className="bottom8"
    ))

#additional settings according to type of chart
additional_settings = dbc.Stack([
    dbc.Col([
        html.Div("x: "), 
        dbc.Select(
            id="x-data",
                ...
                dbc.ModalBody(
                    [
                        dbc.Stack([
                            ...
                            html.Div(radioitems),
                            html.Div(database_server),
                            dbc.Row(server_detail, id="server-detail",style={"display":"none"}),
                            dbc.Row([dbc.Col([html.Div("Select the Widget: "), select], lg=6),
                                     dbc.Col([html.Div("Chart Type: "), select_chart_type], lg=6)]),
                            dbc.Row(additional_settings, id="select-xy-data",style={"display":"none"}),
                            widget_add_alert,
                            dbc.Row([
                                dbc.Col(
...
def creating_new_components(id_new, new_title, interval_value, widget_type, chart_type, x, y):
    ...
    if chart_type == 'pie': chart = pie(id_new, new_title, interval_value)
    elif chart_type == 'table': chart = table(id_new, new_title, interval_value)
    elif chart_type == 'treemap': chart = treemap(id_new, new_title, interval_value)
    elif chart_type == 'sunburst': chart = sunburst(id_new, new_title, interval_value)
    else: 
        chart = scatter(id_new, new_title, interval_value, chart_type, x, y)
    ...
    ], style={"border":"solid 0px lightgrey"}, className="width-100")], 
    id=f'{id_new}')
        ...
        [
            ...
            dtc.ResponsiveGrid(
                ...
                className="ResponsiveStyle grid-line")
                  ...
                  "margin": "5px auto 0", "paddingBottom":"32px"})
...
def myfunc(n_clicks_add,\
    ...
    if clicked == 'add-widget-dashboard':
        ...
        x = None
        y = None
        if xtitle is not None and ytitle is not None:
            mongo = get_cursor(load_info['mongodb'])
            cursor = mongo.Cursor()
            x, y = mongo.xydatas(cursor, xtitle, ytitle)
        ...
        return children + [creating_new_components(new_id, widget_title, interval_value, \
                                                   widget_type, chart_type_value, x, y)], \
...
#show up botton if modification
@app.callback(
...
#show up botton if modification
@app.callback(
    Output("server-detail", "style"),
    Output("select-database-type", "options"),
    Output("select-collection-type", "options"),
    Input("select-server-type", "value"), 
    Input("select-database-type", "value")
    )
def modal_mongo_detail(server, selected_db):

    if server == None:
        raise PreventUpdate

    elif server=="mongodb":
        if not selected_db == None:
            load_info["mongodb"]["mongo_database"] = selected_db
            col = read_mongo(load_info["mongodb"], "colnames")
            return {"display":"grid"}, no_update, col
        else:
            db = read_mongo(load_info["mongodb"], "dbnames")
            return {"display":"grid"}, db, [""]
    else:
        return  {"display":"none"}, [""], [""]
    
#show up selectable xy data
@app.callback(
    Output("select-xy-data", "style"),
    Output("x-data", "options"),
    Output("y-data", "options"),
    Output("x-data", "value"),
    Output("y-data", "value"),
    Input("select-chart-type", "value"), 
    State("select-collection-type", "value")
    )
def additional_datas(chart_type, selected_col):

    if chart_type == None:
        raise PreventUpdate

    elif chart_type=="line":
        load_info["mongodb"]["mongo_col"] = selected_col
        col = read_mongo(load_info["mongodb"], "title")
        return {"display":"grid"}, col, col, no_update, no_update
...
@app.callback(
    ...
    Input('loaded', 'data'))
def loaded_status_info(isLoaded):
